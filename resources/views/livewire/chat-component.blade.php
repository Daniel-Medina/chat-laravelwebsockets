<div class="bg-gray-50 rounded-lg shadow border border-gray-200 overflow-hidden" x-data="data()">
    <div class="grid grid-cols-3 divide-x divide-gray-200">
        ...
        <div class="col-span-1">
            <div class="bg-gray-100 h-16 flex items-center px-4">
                <img src="{{ Auth::user()->profile_photo_url }}"
                    class="object-cover object-center w-10 h-10 rounded-full" alt="{{ auth()->user()->name }}">
            </div>

            <!-- Buscador -->
            <div class="h-14 flex items-center bg-white px-4">
                <x-jet-input type="text" class="w-full" placeholder="Busca un chat o inicia uno nuevo"
                    wire:model="search" />
            </div>
                            ...
                            @forelse ($this->contacts as $contact)
                                <li class="cursor-pointer" wire:click="open_chat_contact({{ $contact }})">
                                    <div class="flex">
                                        ...
                                        <div class="flex-1 ml-4 border-b border-gray-200">
                ...
                @else
                    @foreach ($this->chats as $chatList)
                        <div wire:key="chats-{{ $chatList->id }}"
                            class="flex items-center justify-between {{ $chat && $chat->id == $chatList->id ? 'bg-green-100' : 'bg-white' }} hover:bg-gray-100 cursor-pointer px-3"
                            wire:click="open_chat({{ $chatList }})">

                            <figure>
                                <img src="{{ $chatList->image }}"
                            ...
                            <div class="w-[calc(100%-4rem)] py-4 border-b border-gray-200">
                                <div class="flex justify-between items-center">
                                    ...
                                    <div class="text-right">
                                        ...
                                        @if ($chatList->unread_messages_count)
                                            <span
        ...
        <div class="col-span-2">
            ...
            @if ($contactChat || $chat)
                <!-- Nombre e imagen del chat -->
                <div class="bg-gray-100 h-16 flex items-center px-3">
                    <figure>
                        @if ($chat)
                            <img src="{{ $chat->image }}" class="object-cover object-center w-10 h-10 rounded-full"
                                alt="{{ $chat->name }}">
                        @else
                ...
                <div class="h-[calc(100vh-11rem)] overflow-auto px-3 py-2">
                    ...
                    @foreach ($this->messages as $message)
                        <div class="flex {{ $message->user_id == auth()->user()->id ? 'justify-end' : '' }}">
                            ...
                            <div
                                class="rounded px-3 py-2 {{ $message->user_id == auth()->user()->id ? 'bg-green-100' : 'bg-gray-200' }} mb-2">
                                <p class="text-sm">{{ $message->body }}</p>
                                <p
                                    class="{{ $message->user_id == auth()->user()->id ? 'text-right' : '' }} text-xs mt-1 text-gray-600">
                                    {{ $message->created_at->format('d-m-y h:i A') }}

                                    @if ($message->user_id == auth()->user()->id)
                                        <i
                                            class="fas fa-check-double ml-2 {{ $message->is_read == true ? 'text-blue-500' : 'text-gray-600' }}"></i>
                                    @endif
                                </p>
                ...
                <form class="bg-gray-100 h-16 flex items-center px-4" wire:submit.prevent="sendMessage">
                    <x-jet-input type="text" class="flex-1" placeholder="Escribe un mensaje aqui"
            ...
            @else
                <div class="h-full w-full flex justify-center items-center">
                    <div>
                        <div class="WM0_u" style="opacity: 1;"><span data-testid="intro-md-beta-logo-light"
                                data-icon="intro-md-beta-logo-light" class="IVxyB"><svg width="360"
                                    viewBox="0 0 303 172" fill="none" preserveAspectRatio="xMidYMid meet"
                                    class="">
                                    ...
                                    <rect x=".445" y=".55" width="50.58" height="100.068" rx="7.5"
                                        ...
                                        stroke="#316474"></rect>
                                    <rect x=".445" y=".55" width="50.403" height="99.722"
                                        rx="7.5" transform="rotate(6 -356.664 123.217) skewX(.036)"
                        ...
                        <h1 class="text-center text-gray-500 text-2xl  mt-4">Sitio de Chat</h1>
                    </div>
                </div>
    ...
    @push('js')
        <script>
            ...
            Livewire.on('scrollIntoView', function() {
                document.getElementById("scroll-down").scrollIntoView(true);
            })
        </script>
    @endpush
